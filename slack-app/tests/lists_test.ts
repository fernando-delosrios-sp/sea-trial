...
function buildListClient(overrides: {
  ...
  return { client, createParams, accessParams, itemCreateParams, bookmarkParams };
}

Deno.test("createDeliverablesList attaches bookmark and seeds placeholder row", async () => {
  const {
    client,
    createParams,
    accessParams,
...
Deno.test("createIncidentsList attaches bookmark and seeds placeholder row", async () => {
  const createParams: Array<Record<string, unknown>> = [];
  const bookmarkParams: Array<Record<string, unknown>> = [];
  ...
  assertEquals(createParams[0]?.name, "Incidents");
  assertEquals(bookmarkParams[0]?.channel_id, "C456");
  assertEquals(bookmarkParams[0]?.link, "https://app.slack.com/lists/T01234567/F-incidents");
});
